...
describe("parseWorkspaceSearch", () => {
  ...
  test("checks and release are valid tabs", () => {
    expect(parseWorkspaceSearch({ tab: "checks" }).tab).toBe("checks")
    expect(parseWorkspaceSearch({ tab: "release" }).tab).toBe("release")
  })
...
describe("workspaceHref", () => {
  test("defaults produce a bare incident path", () => {
    expect(workspaceHref("inc-demo-payment-1")).toBe(
    ...
    expect(
      workspaceHref("inc-demo-payment-1", {
        view: "review",
        tab: "summary",
        record: "",
      })
    ).toBe("/incidents/inc-demo-payment-1")
  })

  test("non-default values are included", () => {
    expect(workspaceHref("inc-1", { tab: "files" })).toBe(
      "/incidents/inc-1?tab=files"
    )
    expect(workspaceHref("inc-1", { tab: "checks" })).toBe(
      "/incidents/inc-1?tab=checks"
    )
    expect(workspaceHref("inc-1", { view: "full" })).toBe(
      "/incidents/inc-1?view=full"
    )
    expect(workspaceHref("inc-1", { record: "hypothesis:H1" })).toBe(
      "/incidents/inc-1?record=hypothesis%3AH1"
    )
    expect(
      workspaceHref("inc-1", { tab: "files", record: "remediation" })
    ).toBe("/incidents/inc-1?tab=files&record=remediation")
  })
})
...
describe("resolveRecordId", () => {
  ...
  test("known ids pass through", () => {
    expect(resolveRecordId(available, "remediation", "source-host")).toBe(
      "remediation"
    )
  })

  test("unknown and empty ids fall back to the default record", () => {
    expect(resolveRecordId(available, "evidence:zzz", "source-host")).toBe(
      "source-host"
    )
    expect(resolveRecordId(available, "", "source-host")).toBe("source-host")
    expect(resolveRecordId(available, undefined, "source-host")).toBe(
      "source-host"
    )
  })
})
