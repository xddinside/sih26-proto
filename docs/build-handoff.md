...
1. **[#16 — Contracts, schemas, hashes, journal, and fixture validation](https://github.com/xddinside/sih26-proto/issues/16).** Owns `packages/contracts/`, `demo/fixtures/contracts/`. Inputs: section 9 canonical reports. Outputs: all schemas and types, hash helpers, journal event schema, fixtures. Blocked by: nothing. Accepts: schema tests pass; fixtures validate.
2. **[#14 — Scaffold the frontend and the static replay adapter](https://github.com/xddinside/sih26-proto/issues/14).** Owns the root workspace files, `apps/web/` scaffold, `packages/ui/`, and `apps/web/src/lib/replay/`. Inputs: section 1 command and cleanup list. Outputs: integrated scaffold with no nested `.git`, `node_modules`, or caches; pinned versions; `bun.lock`; one ESLint config; fixed `button.tsx` and `utils.ts` lint errors; Turbo `dist/**` outputs. Blocked by: #16. Accepts: `bun install --frozen-lockfile`, `bun run typecheck`, `bun run lint`, and `bun run build` pass; replay adapter serves the settled bundle layout with manifest, sequence, and hash verification.
3. **[#21 — Incident list, detail, artifact, and saved-truth frontend](https://github.com/xddinside/sih26-proto/issues/21).** Owns `apps/web/src/routes/` and `apps/web/src/features/incidents/`. Inputs: contracts (#16), replay adapter (#14). Outputs: `/`, `/incidents/:id`, and `/incidents/:id/artifacts/:hash` with saved/live labels, provenance strips, receipt-bound numbers, and disabled saved controls. Blocked by: #14. Accepts: saved controls cannot submit; every rendered number has a receipt or saved row.
4. **[#17 — Saved bundles and panel states](https://github.com/xddinside/sih26-proto/issues/17).** Owns `apps/web/src/features/incident-workspace/` and `demo/fixtures/runs/`. Inputs: incident-workspace panel hierarchy. Outputs: panels 1–12, read-only policy, audit tail, telemetry deep links, static Solution Contract panels for rollback and the full review/test catalog. Blocked by: #21. Accepts: pitch-only panels clearly marked; both panels state neither saved run demonstrates rollback; all states render.
5. **[#18 — Astronomy overlay, seeds, rule, and reset](https://github.com/xddinside/sih26-proto/issues/18).** Owns `demo/compose/`, `demo/seeds/`. Inputs: pinned commit, overlay files, seeds. Outputs: healthy Compose overlay, pinned rule file, `S1`/`S2` seeds, capture and reset scripts. Blocked by: #16. Accepts: `card.js` preservation test matches upstream before any seed; rule fires with pinned `rule_version`; seeds apply and reset cleanly.
6. **[#20 — Control Plane, stores, brokers, and local runner](https://github.com/xddinside/sih26-proto/issues/20).** Owns `apps/control-plane/`, `packages/brokers/`, `demo/ci/`. Inputs: contracts (#16), overlay (#18). Outputs: state machine, journal, trigger dedup, gates, local PostgreSQL and artifact storage, Read/Action Brokers, local CI runner, adapters. Blocked by: #16 and #18. Accepts: tests from [orchestrator-stages](research/orchestrator-stages.md) and [worker-isolation](research/worker-isolation.md).
7. **[#15 — Pi Worker, Fusion, repair, reviews, and tests](https://github.com/xddinside/sih26-proto/issues/15).** Owns `packages/pi-skills/core/`, `packages/pi-skills/reviews/`, `packages/pi-skills/tests/`. Inputs: section 10 subset, brokers (#20). Outputs: Worker image, extension, all demo skills. Blocked by: #20. Accepts: allow-list, isolation, round-validity, trace-exclusion, receipt-binding, and deterministic consolidation tests from [pi-agent-catalog](research/pi-agent-catalog.md) and [review-verification](research/review-verification.md).
8. **[#19 — Release Gate, probe ring, Watch, capture, and export](https://github.com/xddinside/sih26-proto/issues/19).** Owns `demo/capture/`, `demo/saved-runs/`. Inputs: panels (#17), Control Plane (#20), skills (#15). Outputs: Run 1 and Run 2 captures, export manifest. Blocked by: #17, #20, and #15. Accepts: Run 1 ends `verified-remediation` and `resolved`, then closed after the confirmation window; Run 2 ends `verification-failed` with no Release record and no production Watch Report; both replay from journal and sealed artifacts alone.
9. **[#22 — Replay, a11y, responsive, screenshots, and rehearsal](https://github.com/xddinside/sih26-proto/issues/22).** Owns `demo/replay/`, `apps/web/e2e/`, and `docs/presentation/`. Inputs: captures (#19), panels (#17). Outputs: replay verification output, 12 screenshots, and the rehearsed script. Blocked by: #17 and #19. Accepts: explicit replay errors for hash mismatch, missing sequence, unknown schema, stale data, redaction, and missing artifact; keyboard, 200% zoom, reduced motion, 1280 px, and 390 px checks; two timed rehearsals; offline presentation.
